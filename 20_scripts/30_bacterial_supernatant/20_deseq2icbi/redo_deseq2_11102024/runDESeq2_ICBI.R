  ...
  if (!is.null(ora_res)) {
    ...
    if (min(res_tab$p.adjust) < 0.05 & length(unique(res_tab$geneID)) > 1) {
      ...
      p = dotplot(ora_res, showCategory=30) +
        theme(plot.margin = unit(c(1, 1, 1, 7), "cm"),  # Maintain the left margin
              axis.text.y = element_text(size = 10, hjust = 1)) +  # Keep the text size
      ...
      p <- cnetplot(ora_res,
                    ...
                    vertex.label.font=6)  + scale_color_gradient2(name="log2FoldChange", low='blue3', high='firebrick')
  ...
  bplapply(names(gsea_tests), function(gsea_name) {
    ...
    if (!is.null(gsea_res)) {
      ...
      if (min(res_tab$p.adjust) < 0.05) {
        ...
        # Create the dotplot with automatic label adjustment
        p = dotplot(gsea_res, showCategory=15, split=".sign") + facet_grid(.~.sign) +
          theme(plot.margin = unit(c(1, 1, 1, 7), "cm"),  # Maintain the left margin
                axis.text.y = element_text(size = 10, hjust = 1)) +  # Keep the text size
          scale_y_discrete(labels = function(x) str_wrap(x, width = wrap_width))  # Dynamic wrapping
...
save_plot(file.path(results_dir, paste0(prefix, "_volcano")), p, width = 8, height = 12)
...
save_plot(file.path(results_dir, paste0(prefix, "_volcano_padj")), p, width = 8, height = 12)
...
if(!is.null(genes_of_interest)) {
  ...
  lapply(goi_groups, function(grp) {
    ...
    save_plot(file.path(results_dir, paste0(prefix, "_", grp_fname, "_volcano_padj")), p,width = 8, height = 12)
